        ...
        payload = json.dumps(
            {
                ...
                "body": {"editor": {"value": html_body, "representation": "editor"},},
            }
        )
    ...
    def embed_math(self, key, value, format, meta):
        ...
        if key == "Math" and value[0]["t"] == "DisplayMath":
            ...
            image_path = self.latex2png(
                latex_eq, f'images/{eq_label.replace(":", "_")}'
            )
            ...
            self._eq_counter += 1
            return Image(
                [eq_label, classes, attrs], inline_elements, [confluence_path, caption]
            )
...
def do_upload(args):
    ...
    pandoc_process = subprocess.run(
        ["pandoc", "-t", "json", in_path], stdout=subprocess.PIPE,
    )
    pandoc_process.check_returncode()
...
def add_download_arguments(parser):
    parser.add_argument(
        "title", help="Title of Confluence page to download",
    )
    parser.add_argument("output", help="File to output page to")
    parser.set_defaults(func=do_download)
    return parser


def add_upload_arguments(parser):
    parser.add_argument("input", help="File to upload")
    parser.add_argument(
        "title", help="Title of Confluence page to upload",
    )
...
def add_arguments(parser):
    parser.add_argument(
        "--config-file", default="~/.config/pandoc-confluence.json",
    )
